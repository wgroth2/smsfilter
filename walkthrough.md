...
* **KDoc & Licensing Standards**: Enforced full BSD 3-Clause licensing headers (authored by Bill Roth <bill.roth@gmail.com>) and mandatory KDoc comments across all Kotlin classes, functions, interfaces, and constants via `AGENTS.md`. This file lives at the **repository root** rather than in a subdirectory because Claude Code only auto-loads `AGENTS.md`/`CLAUDE.md` from the root — in a subdirectory the rules would silently never reach the generation session.
...
1. **Run Phase 1**: Create the Gradle wrapper (Android Studio template or `gradle wrapper --gradle-version 8.11.1` — the wrapper JAR is binary and cannot be AI-generated), then scaffold `libs.versions.toml`, `build.gradle.kts` (declaring **all** catalog dependencies up front so it is never edited again), `AndroidManifest.xml` (including removal of the default WorkManager initializer), the minimal `res/values/` resources the manifest references (AAPT2 hard-fails on a missing `@string`/`@style`; launcher icon PNGs are binary and must be omitted or generated as XML adaptive icons), and the `@HiltAndroidApp Application` class implementing `Configuration.Provider` with `HiltWorkerFactory`. Extend the existing `.gitignore` rather than regenerating it. Verify with `./gradlew assembleDebug`.
