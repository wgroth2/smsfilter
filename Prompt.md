...
If any required permission has been **permanently denied** (the user denied it and Android will no longer show the system dialog — detected via `shouldShowRequestPermissionRationale` returning `false` after a denial), the step must not leave the user stuck on a button that silently does nothing: replace the request button with an explanation and an **"Open App Settings"** button (`Settings.ACTION_APPLICATION_DETAILS_SETTINGS`). When the user returns from App Settings, re-check the permissions and advance automatically if they are now granted.

**Step 3 — Connection Test**
Trigger a real-time connection test to verify access to Google Contacts (by checking if the system contacts database can be queried). Show the connection status result: *"Google Contacts: Accessible"* on success, or *"Google Contacts: Not accessible — permission denied"* (with a note that it can be granted later from Settings) if `READ_CONTACTS` was denied or the query fails. The **"Done" button remains enabled in both cases** — contacts access is non-blocking by design.

Below the connection status, show a brief informational note making the app's default behavior explicit before the user finishes setup: *"Auto-reply is ON by default — SMS Filter will automatically send a one-word 'stop' or 'end' reply when it detects an opt-out message from an unknown sender. You can switch to detection-only mode anytime in Settings."*

"Done" marks `firstRunComplete = true` and navigates to the main Settings screen. Until `firstRunComplete` is `true`, **no incoming SMS may be processed** — see the onboarding gate in Core Components (the manifest-declared receiver is live as soon as `RECEIVE_SMS` is granted mid-wizard, so the worker must enforce this).
...
- **Onboarding gate (before any other processing):** the worker's first action is to read `firstRunComplete` from `DataStore<Preferences>`. If `false`, return `Result.success()` immediately — no lookups, no detection, no reply, no notification, no log entry. This is what enforces "no SMS processing before setup completes": the manifest-declared receiver starts firing as soon as `RECEIVE_SMS` is granted in wizard Step 2, before the user has seen a single settings screen.
...
- **Google Contacts:** Query the system's `ContactsContract.PhoneLookup` using a `ContentResolver` to check if the incoming phone number belongs to a saved contact. Since this uses the Android local Contacts database, it requires `READ_CONTACTS` permission but does not require network calls. **Permission guard (required):** `ContactRepository` must check `checkSelfPermission(READ_CONTACTS)` before every query and return "not found" (treat the sender as unknown) when the permission is missing — it must never let a `SecurityException` reach the worker. This path is reachable in normal use: onboarding explicitly allows finishing with `READ_CONTACTS` denied.
...
- Rendered at the very top of the Settings screen, showing real-time connection status indicators (active/disconnected) with colored dots:
  ...
  - **HubSpot CRM**: Gray dot ("Off") when the "Use HubSpot" toggle is off, Amber dot ("Setup incomplete") when the toggle is on but no token has been saved yet (e.g., the user tapped "Connect" in the post-onboarding dialog and then abandoned the token field), Green dot ("Connected") when connected, or Red dot ("Token invalid / Unreachable") only when the toggle is on, a token is saved, and the last check failed. Neither a deliberately-disconnected HubSpot nor an unfinished setup may ever be styled as an error state.
...
| 18 | Unknown | "Hello\nSTOP" received mid-wizard (before `firstRunComplete` is set) | Nothing — no reply, no notification, no log entry (onboarding gate) |
| 19 | Unknown | "Hello\nSTOP" with `READ_CONTACTS` denied | Alert and auto-reply "stop" — no crash; Google Contacts lookup returns "not found" without querying |
...
- **EncryptedSharedPreferences is a deliberate, accepted choice.** The `androidx.security:security-crypto` library is deprecated by Google and `1.1.0-alpha06` is its final release. It remains fully functional on API 26–35 and is intentionally retained here for a single-user sideloaded app — during code generation, do **not** substitute a different secure-storage library, add new dependencies, or "upgrade" this choice; use it exactly as pinned in the Dependency Version Catalog.
...
- **Database Migration Strategy**: To prevent crashes during development and updates, configure the Room database builder with `fallbackToDestructiveMigration(dropAllTables = true)` so that the database schema is safely recreated if changed, without requiring manual migration scripts. (Use this exact overload — the no-argument `fallbackToDestructiveMigration()` is deprecated in Room 2.7.)
...
- **Installation Documentation**: A comprehensive installation guide (`INSTALL_GUIDE.md`) must be generated for the web/users. This guide must explain step-by-step how to download the APK, enable the "Install unknown apps" permission for browsers/file managers, and bypass/resolve standard Google Play Protect warnings for sideloaded apps. It must also include a short "Things to know" section covering two Android platform behaviors that are not bugs: (1) **force-stopping the app** (from App Info) suspends the SMS receiver until the app is next opened manually — users should simply not force-stop it; (2) on some OEM devices (Samsung, Xiaomi), aggressive battery management can delay background processing by seconds to minutes — adding the app to the battery-optimization exception list restores near-immediate handling.
...
1. **Phase 1 — Project Scaffolding & Build Configuration**
   - **Create the Gradle wrapper first.** The wrapper JAR (`gradle/wrapper/gradle-wrapper.jar`) is a binary file that an AI cannot author — obtain it either by scaffolding the project from an Android Studio "Empty Activity" template or by running `gradle wrapper --gradle-version 8.11.1` with a locally installed Gradle (AGP 8.10 requires Gradle 8.11.1 or newer). **On Gradle 9.x a minimal `settings.gradle.kts` must exist before the wrapper task will run** — 9.x refuses to execute any task in a directory that is not already a Gradle build, so create a one-line stub (`rootProject.name = "smsfilter"`) first; this phase then replaces it with the real settings file. Commit `gradlew`, `gradlew.bat`, and `gradle/wrapper/` to git; no phase verification can run without them.
   - Generate `gradle/libs.versions.toml` using **exactly** the pinned versions from the Dependency Version Catalog section above.
   - Generate `build.gradle.kts` (root & app module), `settings.gradle.kts`, `gradle.properties`, `proguard-rules.pro`, and `AndroidManifest.xml` (permissions, receiver, and service declarations — these are forward declarations for classes generated in later phases).
   - **`gradle.properties` must set `android.useAndroidX=true`.** This file is normally supplied by the Android Studio project template, so it is easy to overlook when scaffolding by hand — and without it the build fails at `:app:checkDebugAarMetadata` with *"Configuration contains AndroidX dependencies, but the `android.useAndroidX` property is not enabled."* Also set `android.nonTransitiveRClass=true` and an `org.gradle.jvmargs` heap large enough for KSP, Hilt, and Room annotation processing (2 GB is sufficient).
   - **The app module's `build.gradle.kts` must declare every dependency in the Dependency Version Catalog in this phase — not incrementally as later phases need them.** All versions are already pinned, unused KSP processors are harmless no-ops, and R8 strips unused code from release builds. Declaring them up front means `build.gradle.kts` is written exactly once and is never edited again in Phases 2–7, which preserves the rule that a phase does not modify files generated by earlier phases.
   - **`.gitignore` already exists in the repository and must be extended, never regenerated.** It contains project-specific rules (e.g. `chat.id`, `pdf/`) that overwriting would silently delete. Add any missing Android/Gradle entries; leave every existing line intact.
   - **Generate the minimal `res/` resources the manifest references.** `AndroidManifest.xml` is resolved by AAPT2 at build time, so a reference to a resource that does not exist is a hard build failure (`error: resource string/app_name not found`), not a warning. At minimum generate `res/values/strings.xml` (containing `app_name`) and `res/values/themes.xml` (a Material3 theme matching the `android:theme` the manifest declares). **Launcher icons are binary assets an AI cannot author** — the same constraint as the Gradle wrapper JAR — so resolve `android:icon` one of two ways: either omit the `android:icon` attribute entirely in this phase and let the system default stand until a real icon is added in Android Studio, or generate a pure-XML adaptive icon (`res/mipmap-anydpi-v26/ic_launcher.xml` plus a vector-drawable foreground and a background color). Do not reference a `@mipmap/ic_launcher` PNG that does not exist.
   ...
   - *Verification:* Confirm Gradle syncs cleanly and `./gradlew assembleDebug` compiles the bare application skeleton with no errors. Note that `MainActivity` is forward-declared in the manifest but not implemented until Phase 6, so the APK builds and installs but will crash on launch until then — this is expected, not a build defect.

2. **Phase 2 — Room Database, DataStore & Secure Storage**
   - Implement all Room entities — `StopListEntity`, `OptOutPatternEntity` (pattern, `replyType`, `matchMode`), `DetectionLogEntity`, and `AutoReplyCooldownEntity` (`senderHash` SHA-256 primary key, `lastReplyTimestamp`) — their DAOs, and the Room database class configured with `fallbackToDestructiveMigration(dropAllTables = true)` (the no-arg overload is deprecated in Room 2.7).
...
4. **Phase 4 — Background SMS Pipeline**
   - Implement `ContactRepository` (Google Contacts `ContactsContract.PhoneLookup` via `ContentResolver` + 15-minute in-memory `LruCache`), including the mandatory `checkSelfPermission(READ_CONTACTS)` guard: when the permission is missing, return "not found" instead of querying — never let a `SecurityException` propagate to the worker.
   ...
   - Implement `SmsLookupWorker` as an Expedited Work Request with `getForegroundInfo()` fallback notification. The worker must inject the **`SettingsDataStore` wrapper** (generated in Phase 2) and read the following five values on `Dispatchers.IO` **before any processing logic runs**:
     - `firstRunComplete: Boolean` — the onboarding gate: if `false`, return `Result.success()` immediately with no lookups, detection, reply, notification, or log write. The manifest receiver is live as soon as `RECEIVE_SMS` is granted mid-wizard, so this check is what enforces "no SMS processing before setup completes."
...
6. **Phase 6 — Onboarding UI & Permissions Screen**
   ...
   - Implement `PermissionsScreen.kt` with `ActivityResultContracts.RequestMultiplePermissions`, rationale dialogs, permission denial banners, and the permanently-denied escape hatch: when `shouldShowRequestPermissionRationale` returns `false` after a denial, show an "Open App Settings" button (`Settings.ACTION_APPLICATION_DETAILS_SETTINGS`) and re-check permissions when the user returns.
